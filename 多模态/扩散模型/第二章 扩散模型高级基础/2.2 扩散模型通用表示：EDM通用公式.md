...
本系列源自Nvidia的论文 **《Elucidating the Design Space of Diffusion-Based Generative Models》** ，后面简称EDM。
...
$$\begin{align}
\mathbf{x}_t &= \mathbf{x}_0 + σ_t \epsilon \\
p(\mathbf{x}_t|\mathbf{x}_0) &= \mathcal{N} (\mathbf{x}_t; \mathbf{x}_0, σ^2_t \mathbf{I})
\end{align}$$
如果使用DDPM做一步加噪公式，可写成如下概率分布形式：
$$\begin{align}
\mathbf{x}_t &= \sqrt{\bar{\alpha}_t}\mathbf{x}_0 + \sqrt{1 − \bar{\alpha}_t} \epsilon \\
p(\mathbf{x}_t|\mathbf{x}_0) &= \mathcal{N} (\mathbf{x}_t;\sqrt{\bar{\alpha}_t}\mathbf{x}_0,\sqrt{1 − \bar{\alpha}_t}\mathbf{I})
\end{align}$$
EDM提出，扩散模型的通用加噪公式为：
$$p(\mathbf{x}_t|\mathbf{x}_0) = \mathcal{N} (\mathbf{x}_t; s(t)\mathbf{x}_0, s^2(t)σ^2(t)\mathbf{I}) \tag{2.7}$$
...
> [!Info]
> 
> 在介绍通用采样方法之前，我们发现一个问题：从 $s(t),σ(t)$ 算到 $f(t),g(t)$ 需要复杂的积分不太容易。既然 $s(t),σ(t)$ 系统与 $f(t),g(t)$ 系统相互独立且等价。我们是否可以抛弃 $f(t),g(t)$ 系统，直接用 $s(t),σ(t)$ 系统呢？完全可以！这个思路也是下面我们主要的推理过程。这里还补一下 $s(t),σ(t)$ 系统到 $f(t),g(t)$ 系统的转换公式，结论在公式2.8，2.9中：
> $$\begin{align}
s(t)&=e^{\int_0^tf(u)\mathrm{d}u} \\
\ln s(t) &= \int_0^tf(u)\mathrm{d}u \\
f(t) &= \frac{s'(t)}{s(t)} \tag{2.8} \\
σ(t) &= \sqrt{\int_0^t \frac{g^2(u)}{s^2(u)}\mathrm{d}u} \\
σ^2(t) &= \int_0^t \frac{g^2(u)}{s^2(u)}\mathrm{d}u \\
2σ(t)σ'(t)&=\frac{g^2(t)}{s^2(t)} \\
g(t)&=s(t)\sqrt{2σ(t)σ'(t)} \tag{2.9}
\end{align}$$

### 2.2.2 基于PF-ODE的通用采样公式
根据福克-普朗克方程，我们知道SDE有一个**没有随机项**并且**边缘分布相同**的等价形式PF-ODE，即公式2.4：
$$\mathrm{d}\mathbf{x}=\big[f(\mathbf{x},t)- \frac{1}{2}g^2(t) \nabla_{\mathbf{x}_t} \log p_t(\mathbf{x}_t) \big]\mathrm{d}t \tag{2.4}$$
...
$$\begin{align}
p(\mathbf{x}_t) &= \int_{\mathbb{R}^d} p_\text{data}(\mathbf{x}_0)\mathcal{N} (\mathbf{x}_t; s(t)\mathbf{x}_0, s^2(t)σ^2(t)\mathbf{I})\mathrm{d} \mathbf{x}_0 \\
&= s^{-d}(t) \int_{\mathbb{R}^d} p_\text{data}(\mathbf{x}_0) \mathcal{N}(\frac{\mathbf{x}_t}{s(t)}-\mathbf{x}_0;0,σ^2(t)\mathbf{I})\mathrm{d} \mathbf{x}_0 \tag{2.10} 
\end{align}$$
公式2.10 是典型的对正态分布标准化的处理方式。使用两个随机分布的卷积公式（不要理解成CNN的卷积公式）相加积分内元素，得：
$$\begin{align}
p(\mathbf{x}_t) &= s^{-d}(t) \underbrace{\big[p_\text{data} * \mathcal{N}(\frac{\mathbf{x}_t}{s(t)}-\mathbf{x}_0;0,σ^2(t)\mathbf{I})\big]}_{这一串是卷积操作，本质是函数名} \underbrace{\big( \frac{\mathbf{x}_t}{s(t)} \big)}_{这是函数参数}\tag{2.11} 
\end{align}$$
...
在公式2.11中，我们发现 $p_\text{data}$ 函数本质上是原图 $\mathbf{x}_0$ ，我们可以认为 $p_\text{data}$ 是一个点 $(0,x_0)$ ，在这个点下我们可以认为 $p_\text{data} \sim \mathcal{N}(\mathbf{x}_0,σ^2(t)\mathbf{I})=\mathcal{N}(\mathbf{x}_0;0,σ^2(t)\mathbf{I})$ ，这里的方差是反而不重要，因为我们就用一个点。写成这种格式是要做正态分布结合。因此可以算出两个正态分布结合的正态分布为:
$$\begin{align}
p_\text{data} * \mathcal{N}(\frac{\mathbf{x}_t}{s(t)}-\mathbf{x}_0;0,σ^2(t)\mathbf{I}) &= \mathcal{N}(\mathbf{x}_0;0,σ^2(t)\mathbf{I})* \mathcal{N}(\frac{\mathbf{x}_t}{s(t)}-\mathbf{x}_0;0,σ^2(t)\mathbf{I})\\
&\sim \mathcal{N}(\frac{\mathbf{x}_t}{s(t)};0,σ^2(t)\mathbf{I}) \\
p(\mathbf{x}_t) &= s^{-d}(t) \mathcal{N}(\frac{\mathbf{x}_t}{s(t)};0,σ^2(t)\mathbf{I})
\end{align}$$
...
$$\begin{align}
\nabla_{\mathbf{x}_t} \log p_t(\mathbf{x}_t) &= \nabla_{\mathbf{x}_t} \log [ s^{-d}(t) \mathcal{N}(\frac{\mathbf{x}_t}{s(t)};0,σ^2(t)\mathbf{I})] \\
&=\nabla_{\mathbf{x}_t} \log s^{-d}(t) + \nabla_{\mathbf{x}_t} \log p(\frac{\mathbf{x}_t}{s(t)};σ(t)) \\
&= \nabla_{\mathbf{x}_t} \log p(\frac{\mathbf{x}_t}{s(t)};σ(t))
\end{align}$$

再结合公式2.8、公式2.9，可以直接得出 PF-ODE为以下形式：
$$\mathrm{d}\mathbf{x}_t=\big[\frac{s'(t)}{s(t)}\mathbf{x}_t - s^2(t)σ(t)σ'(t)\nabla_{\mathbf{x}_t} \log p(\frac{\mathbf{x}_t}{s(t)};σ(t))\big]\mathrm{d}t \tag{2.12}$$
从现在开始，我们参照公式2.12，就可以针对任意⼀个已知 $s(t),σ(t)$ 的扩散公式，直接写出对应的 PF-ODE 去噪采样迭代公式了。
...
这里使用 $D(\mathbf{x};σ(t))$ 来记录这种去噪模型。这也就说明，公式2.12并不是EDM的重点，不能直接用于采样。下面的部分我们要介绍更重要的公式。
...
$$\begin{align}
\nabla_{\mathbf{x}_t} \log p_t(\mathbf{x}_t) = \nabla_{\mathbf{x}_t} \log p(\frac{\mathbf{x}_t}{s(t)};σ(t)) &=  \nabla_{\mathbf{x}_t} \log \mathcal{N}(\frac{\mathbf{x}_t}{s(t)};0,σ^2(t))\\ 
&=\frac{D_\theta(\frac{\mathbf{x}_t}{s(t)};σ(t))-\frac{\mathbf{x}_t}{s(t)}}{s(t)σ^2(t)} \tag{2.13}
\end{align}$$
>[!不严谨理解]
>公式2.13在EDM的推理过程很复杂，但是我们突然发现这实际上是一轮正态函数处理。我们发现，引入噪声后，分数函数由 $\nabla_{\mathbf{x}_t} \log p_t(\mathbf{x}_t)$ 变成了 $\nabla_{\mathbf{x}_t} \log p_t(\mathbf{x}_t|\mathbf{y})$ 。整体分布相当于从0为均值变化成为 $\mathbf{y}$ 是均值。 因为我们的加噪起点完全变掉了。我们设 $\hat{\mathbf{x}}_t=\frac{\mathbf{x}_t}{s(t)}$  则有：
>$$\begin{align}
...
\end{align}$$
如果是连续迭代做去噪到最后操作，那么 $\mathbf{y} + \mathbf{n}$ 所对应的的“原加噪变量”就可以是 $\hat{\mathbf{x}}_t$，即为公式2.13的格式。同时，上式也是DDPM与SMLD的“噪声模型-分数模型”核心转换公式 $\nabla _{\tilde{x}} \log q_σ(\tilde{x}|x)=- \frac{\tilde{x} - x}{𝜎^2}$ 的统一。

如果将公式2.13 带入PF-ODE，则可以给出去噪采样的通用形式：
$$\mathrm{d}\mathbf{x}_t=\big[(\frac{s'(t)}{s(t)}+\frac{σ'(t)}{σ(t)})\mathbf{x}_t - \frac{s(t)σ'(t)}{σ(t)}D_\theta\big(\frac{\mathbf{x}_t}{s(t)};σ(t)\big) \big]\mathrm{d}t \tag{2.14}$$

根据公式2.14，我们可以得到如下通用采样算法伪代码
![](../images/2.2.png)
