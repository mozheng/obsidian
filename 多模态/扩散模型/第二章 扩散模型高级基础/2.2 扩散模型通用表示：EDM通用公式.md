...
$$\begin{align}
p(\mathbf{x}_t) &= s^{-d}(t) \underbrace{\big[p_\text{data} * \mathcal{N}(\frac{\mathbf{x}_t}{s(t)}-\mathbf{x}_0;0,σ^2(t)\mathbf{I})\big]}_{这一串是卷积操作} \underbrace{\big( \frac{\mathbf{x}_t}{s(t)} \big)}_{这是卷积函数参数}\tag{2.11} 
\end{align}$$
...
在公式2.11中，我们发现 $p_\text{data}$ 函数本质上是原图 $\mathbf{x}_0$ ，我们可以认为 $p_\text{data}$ 是一个点 $(0,x_0)$ ，使用狄拉克 δ 函数表示。
...
$$\begin{align}
p_\text{data} * \mathcal{N}(\frac{\mathbf{x}_t}{s(t)}-\mathbf{x}_0;0,) &= \mathcal{N}(\mathbf{x}_0,\xi)* \mathcal{N}(\frac{\mathbf{x}_t}{s(t)}-\mathbf{x}_0;0,σ^2(t)\mathbf{I})\\
