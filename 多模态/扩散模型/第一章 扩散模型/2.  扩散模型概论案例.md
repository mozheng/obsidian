...
所以最后的loss是第二和第三项的和：
$$\begin{align*} \mathcal{L} &= \int p({x}) ||{s} _{\theta}({x})||^{2} d {x} + 2\int p({x}) \text{tr}(\nabla _{{x}}{s}_{\theta}({x})) d {x}\\\\ &= \mathbb{E}_{p({x})}[||{s} _{\theta}({x})||^{2} + 2\text{tr}(\nabla _{{x}}{s}_{\theta}({x}))].\end{align*}$$
这里再给大家说下，$tr(.)$ 到底是怎么求？
...
定义一组几何级数序列 $\left\{ \sigma _i \right\} _{i=1}^{L}$ ，其中 $\sigma _i>0$ 且 $\frac{\sigma _1}{\sigma _2}=\frac{\sigma _2}{\sigma _3}=\cdots =\frac{\sigma _{L-1}}{\sigma _L}>1$ ，用分布 $q_{\sigma}\left( {x} \right) =\int{p_{data}\left( {x} \right) \mathcal{N} \left( {x}|{t},\sigma ^2{I} \right) \mathrm{d}{t}}$ （暂时不用理解为什么）表示扰动噪声数据分布。NCSN的目标是训练一个条件分数网络 $s_{\theta}\left( {x};t \right)$ 来估计扰动数据的分布，即 $\forall {{{\sigma }}_{i}}\in \left\{ \sigma _i \right\} _{i=1}^{L}$ 都有 $s_{\theta}\left( {x},\sigma \right) \approx \nabla _x\log q_{\sigma}\left( {x} \right)$ ，其中当 ${x}\in \mathbb{R} ^D$ 时 $s_{\theta}\left( {x},\sigma \right) \in \mathbb{R} ^D$ 。一般把 $s_{\theta}\left( {x};t \right)$ 称为噪声条件分数网络。
